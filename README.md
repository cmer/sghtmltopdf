...
設計は[0048](docs/decisions/0048-margin-collapse-parent-child-empty-design.md)。

* 親に`border-top`/`padding-top`が無ければ、最初の子の`margin-top`が親を
  突き抜けて相殺する(下側も対称。`<div>`でラップした段落の上に余分な余白が
  出なくなる)
* 空ブロック(高さ0・`border`/`padding`無し)の上下マージンは1つに相殺され、
  二重に効かない
* レイアウト結果の実効マージンを見て相殺するため、多階層の相殺につながる
* 既知の限界: 親子相殺は`height: auto`のブロックのみ。`overflow`による
  BFC確立での相殺抑制は非対応(実務では稀)
