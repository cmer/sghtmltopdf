...
グリフのまま(選択・検索できるテキストとして)埋め込みます。

このとき使えるフォントは**文書が使うフォントそのもの**です。`--font`で渡した
フォントも`@font-face`で読み込んだフォントも、そのままSVGの中から引けます。
...
`font-family`を書かなかった`<text>`は、文書の既定フォント(`--font`で最初に
渡したもの)で描きます。

* 文書が持っていないfamilyは、勝手に別のフォントで代用しません。SVGのために
  システムフォントを探し直すことはしないので、`<img>`から参照したSVGが
  文書と違う字で出ることはありません
