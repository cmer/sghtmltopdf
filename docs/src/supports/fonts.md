...
絵文字はモノクロのアウトラインフォントで描きます。
GoogleのNoto Emoji(カラーのNoto Color Emojiではなく、アウトライン版)を指定すれば、そのまま描画されます。

```sh
sghtmltopdf report.html --font NotoEmoji-Regular.ttf
```

カラーフォントは非対応です。
`CBDT`/`CBLC`・`COLR`/`CPAL`・`sbix`のいずれも扱いません。

この種のフォントは指定しても使われず、警告が出ます。

```
警告: NotoColorEmoji.ttf は輪郭を持たない(ビットマップのカラー絵文字専用の)
  フォントのため使用しません。カラーフォントは未対応です。
  絵文字にはモノクロのアウトライン版(Noto Emojiなど)を指定してください
```

黙って無視するのではなく採用しないのは、輪郭を持たないフォントを埋め込むと、文字が豆腐にすらならず完全に消えたうえ、サブセット化が効かずPDFだけが数MB膨らむためです。
不採用にすれば通常のフォント探索へ落ち、描画できる文字が無ければ「豆腐になります」の警告が文字を名指しして出ます。
