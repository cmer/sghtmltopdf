...
|---|---|---|---|
| 1,000 | 48KB | 22MB / 1.4秒 | 10MB / 1.4秒 |
| 5,000 | 236KB | 72MB / 7.0秒 | 11MB / 8.6秒 |
| 20,000 | 948KB | 264MB / 30.1秒 | 15MB / 29.2秒 |
| 60,000 | 2.8MB | 825MB / 92.8秒 | 27MB / 104.6秒 |

通常モードのピークメモリは文書サイズにほぼ比例して増えますが、`--streaming`ではほとんど増えません。
60,000要素では約30分の1になります。
`--streaming`でもわずかに増えるのは、PDFの相互参照表(全オブジェクトの位置)と使用グリフの集計を最後まで保持するためです。

処理時間は両モードでほぼ変わりません。
この計測では`--streaming`が5,000要素で2割ほど、60,000要素で1割ほど遅く、20,000要素ではほぼ同じでした。
メモリを大きく減らす代わりに時間を大きく損なう、という関係にはなっていません。

測定条件は次のとおりです。

* sghtmltopdf 0.1.0のreleaseビルド、Intel Core Ultra 7 258V / メモリ16GB / WSL2(Linux 5.15)
* 高さ60pxの`<p>`を要素数ぶん並べただけのHTML。フォントは`--font`で明示
* ピークメモリは`/usr/bin/time`が報告する最大常駐セットサイズ(RSS)。2回実行して良いほうの値
